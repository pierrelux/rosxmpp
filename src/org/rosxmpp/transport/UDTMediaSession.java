...
public class UDTMediaSession extends JingleMediaSession {
    ...
    private boolean transmitActive = false;

    private static Logger logger = Logger.getLogger("org.rosxmpp.cli");

    /**
     * Creates a TestMediaSession with defined payload type, remote and local
     * candidates
     * 
     * @param payloadType
     *            Payload of the jmf
     * @param remote
     *            the remote information. The candidate that the jmf will be
     *            sent to.
     * @param local
     *            the local information. The candidate that will receive the jmf
     * @param locator
     *            media locator
     */
    public UDTMediaSession(final PayloadType payloadType,
	...
	if ((session != null)
		&& (session.getInitiator().equals(session.getConnection()
			.getUser()))) {
	    // If the initiator of the jingle session is us then we transmit a
	    // UDT stream.
	    try {
    ...
    public void setLocalTopicPublisherAddress(String host, int port) {
	publisherPort = port;
	publisherHost = host;
    }
	...
	if (transmitActive) {
	    logger.info("Socket active, start transmission");
	    try {
		new Socket(publisherHost, publisherPort);
